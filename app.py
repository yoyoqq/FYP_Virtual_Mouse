...
class Scroll():
    def __init__(self) -> None:
        ...
        self.up = -30
        self.delay = 0
        self.scroll_delay = [0, 1] # current delay, total delay
        
    def __scroll_delay(self) -> bool:
        if self.scroll_delay[0] == self.scroll_delay[1]:
            self.scroll_delay[0] = 0
            return True
        self.scroll_delay[0] += 1
        return False
        
    def scrollDown(self):
        if self.__scroll_delay():
            self.down += 5
            return self.down
        else: return 0
